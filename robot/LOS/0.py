...
def arm_and_takeoff(vehicles, aTargetAltitude):
    ...
    while not vehicles.is_armable:
        ...
        time.sleep(1)
    print("Arming motors")
    vehicles.mode = VehicleMode("MANUAL")
    vehicles.armed = True
    while not vehicles.armed:
        print(" Waiting for arming...")
        time.sleep(1)
    print("Taking off!")
    vehicles.simple_takeoff(aTargetAltitude)
    time.sleep(1)
...
if __name__ == '__main__':
    connection_string = '127.0.0.1:14550'  # /dev/serial0
    ...
    clientSocket.bind(('192.168.253.128', 9990))  # server
    clientSocket.connect(('192.168.253.1', 10000))  # client
    ...
    while 1:
        ...
        # c = TS*(vs/r) + TS*k*(vs/r)*mt.tanh(e[0][0]) + c -TS*k*mt.tanh(c-cs[1]+c-cs[2])
        c = TS * (vs / r) + c
