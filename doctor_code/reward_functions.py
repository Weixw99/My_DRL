...
class RewardFunctions:
    ...
    def __init__(self, x_index, y_index, fi_index, u_index, v_index, r_index ):
...
#            if(self.which_agent==2):
                reward_func= self.usv_follow_traj
...
######################################################################################################################
    ...
    def swimmer_follow_traj(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward,
        ...
        diff = np.abs(pt[:,self.fi_index]-angle)
        ...
        # prevent speed away from 0.3
        self.speed_penalty_factor = 2
        diff2 = np.abs(pt[:, self.u_index] - 0.4)
#        my_range1 = 0.03
#        scores[diff2 < my_range1] += (self.speed_penalty_factor * diff2)[diff2 < my_range1]
#        scores[diff2 >= my_range1] += 15
...
#        scores[limit<second_joint] += 2
    ...
    def usv_follow_traj(self, pt, prev_pt, scores, curr_seg, done_forever, all_samples, pt_number):
...
#        scores[moved_to_next == 1] -= self.forward_encouragement_factor * (curr_forward)[moved_to_next == 1]
        ...
        # angle that (desired traj) line segment makes WRT horizontal
        curr_line_start = self.desired_states[curr_seg  ]
        #print("self.desired_states[curr_seg  ]为：",self.desired_states[curr_seg  ])  # 这个是变化的

        #curr_line_end = self.desired_states[curr_seg + 1]
        x_r = curr_line_start[:,0]
        y_r = curr_line_start[:,1]
        #angle = (curr_seg   ) * 0.0012 + 0.5 * np.pi
        angle = curr_line_start[:,2]
#        angle = np.arctan2(curr_line_end[:, 1] - curr_line_start[:, 1], curr_line_end[:, 0] - curr_line_start[:, 0])
#        print (angle[0])
        # ^ -pi to pi
#        if(angle)
...
#        scores[diff < my_range] += (self.heading_penalty_factor * diff)[diff < my_range]
#        scores[diff >= my_range] += 20
        scores += self.horiz_penalty_factor * np.multiply((pt[:,self.x_index]-x_r),(pt[:,self.x_index]-x_r))
        scores += self.forward_encouragement_factor * np.multiply((pt[:,self.y_index]-y_r),(pt[:,self.y_index]-y_r))
        scores += self.heading_penalty_factor * np.multiply((pt[:,self.fi_index]-angle),(pt[:,self.fi_index]-angle))
        scores += self.r_penalty_factor*np.multiply((pt[:,self.r_index]-0.01),(pt[:,self.r_index]-0.01))
#        scores = -1*scores
        # prevent speed away from 0.3
        #self.speed_penalty_factor = 2
        #diff2 = np.abs(pt[:, self.u_index] - 0.4)
