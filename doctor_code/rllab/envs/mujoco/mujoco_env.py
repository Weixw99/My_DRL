...
class MujocoEnv(Env):
    ...
    def __init__(self, action_noise=0.0, file_path=None, template_args=None):
        ...
        self.qacc_dim = self.init_qacc.size #added this
    ...
    def reset_mujoco(self, init_state=None, evaluating=None, isSwimmer=False, need_diff_headings=False):

        if(evaluating):
            if(isSwimmer):
                ##print("Resetting env in a way similar to open ai gym reset func.")
                self.model.data.qpos = self.init_qpos + np.random.uniform(low=-.1, high=.1, size=self.init_qpos.shape)
                self.model.data.qvel = self.init_qvel + np.random.uniform(low=-.1, high=.1, size=self.init_qvel.shape)
                self.model.data.qacc = self.init_qacc
                self.model.data.ctrl = self.init_ctrl
                starting_state=np.squeeze(np.concatenate((self.model.data.qpos, self.model.data.qvel, self.model.data.qacc, self.model.data.ctrl)))
            else:
                self.model.data.qpos = self.init_qpos + np.random.uniform(low=-.1, high=.1, size=self.init_qpos.shape)
                self.model.data.qvel = self.init_qvel + np.random.randn(self.init_qvel.shape[0], self.init_qvel.shape[1]) * .1
                self.model.data.qacc = self.init_qacc
                self.model.data.ctrl = self.init_ctrl
                starting_state=np.squeeze(np.concatenate((self.model.data.qpos, self.model.data.qvel, self.model.data.qacc, self.model.data.ctrl)))
            ...
            #set the model qpos
            if(isSwimmer):
                if(need_diff_headings):
                    my_values = self.init_qpos + np.random.normal(size=self.init_qpos.shape) * 0.5 #0.1
                    my_values[2] = np.random.uniform(-math.pi, math.pi) #heading...
                    self.model.data.qpos = np.copy(my_values) 
                else:
                    self.model.data.qpos = self.init_qpos + np.random.normal(size=self.init_qpos.shape) * 0.01
            else:
                self.model.data.qpos = self.init_qpos + np.random.normal(size=self.init_qpos.shape) * 0.01

            #set the other model vals
            ...
            for datum_name in ["qpos", "qvel", "qacc", "ctrl"]:
                ...
                start += datum_dim
            starting_state = init_state
        return starting_state

    @overrides
    def reset(self, init_state=None, evaluating=False, returnStartState=False, isSwimmer=False, need_diff_headings=False): #### anusha edited this
        starting_state = self.reset_mujoco(init_state, evaluating, isSwimmer, need_diff_headings)

        self.model.forward()
        self.current_com = self.model.data.com_subtree[0]
        self.dcom = np.zeros_like(self.current_com)
        if(returnStartState):
            return self.get_current_obs(), starting_state
        else:
