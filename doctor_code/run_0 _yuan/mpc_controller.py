...
#        observations = np.copy(curr_state)
        ...
        while(stop_taking_steps==False):
            ...
            #get optimal action
            best_action, best_sim_number, best_sequence, moved_to_next, best_curr_seg = self.get_action(curr_state, curr_line_segment, reward_func)
...
#                action_to_take[1] = np.clip(action_to_take[1], -1, 1)
            ...
            for step_num in range(self.horizons):
...
#            print ("###nextstate###",next_state[1].tolist(),"####nextstate###")
            ...
            if(follow_trajectories):
                    if((step>=self.steps_per_episode) or (curr_line_segment>5800)): #6289
                        stop_taking_steps = True
#            else:
...
#        env_infos=tensor_utils.stack_tensor_dict_list(env_infos))
        print (step)
        return traj_taken, actions_taken ##, mydict  #total_reward_for_episode, 

    def get_action(self, curr_nn_state, curr_line_segment, reward_func):   ###curr_nn_state (1,8),xy,fi,u,v,r,sin,cos
...
####################################重复第一组序列，#######################
        all_samples_Tu = npr.uniform(0, 2, (self.N, 1 , 1))
        all_samples_Tr = npr.uniform(-1.5,1.5, (self.N, 1, 1))
...
##3-13            prev_forward = np.copy(curr_forward)
        ...
        return best_action, best_sim_number, best_sequence, moved_to_next,best_curr_seg
