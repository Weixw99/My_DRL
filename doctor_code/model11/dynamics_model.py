...
#        self.opt = tf.train.GradientDescentOptimizer(learning_rate)
    ...
    def train(self, dataX, dataZ, dataX_new, dataZ_new, nEpoch, save_dir, fraction_use_new): ###new:old=fraction_use_new*10:1
        ...
        start = time.time()
        training_loss_list = []
        range_of_indeces = np.arange(dataX.shape[0])    ##8300
        nData_old = dataX.shape[0]                      ##8300
...
###########################################################
        ...
        for i in range(nEpoch):
            ...
            #train completely from new set
                ...
                for batch in range(int(math.floor(num_new_pts / batchsize_new_pts))):  ####batch=iteration
...
#            print("###########################curr_state############", curr_states.shape)# [0:6])
