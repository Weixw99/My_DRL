...
def generate_training_data_inputs(states0, controls0):
    # init vars
    print(len(states0),len(states0[0]),len(states0[0][0]))
    print("####################################################%$%#%$#$%#$%##########")
    print(len(controls0),len(controls0[0]),len(controls0[0][0])) 
    states=np.copy(states0)
    controls=np.copy(controls0)
    new_states=[]
    new_controls=[]
    print(states.shape[0],len(states),states.shape[1],states.shape[2])
    print("####################################################################")
    print(controls.shape[0],len(controls),controls.shape[1],controls.shape[2]) 
    ...
    np.save( '/home/sqy/nn_dynamics/run_0/data_genatesss/states.npy', new_states)
    np.save( '/home/sqy/nn_dynamics/run_0/data_genatesss/controls.npy', new_controls)
    print(len(new_states),len(new_states[0]),len(new_states[0][0]))
    print("########################################newnewnewnew############################")
    print(len(new_controls),len(new_controls[0]),len(new_controls[0][0])) 
    ...
    np.save('/home/sqy/nn_dynamics/run_0/data_genatesss/dataX.npy', dataX)
    np.save('/home/sqy/nn_dynamics/run_0/data_genatesss/dataY.npy', dataY)
    return dataX, dataY
...
def generate_training_data_outputs(states, which_agent):
    ...
    np.save('/home/sqy/nn_dynamics/run_0/data_genatesss/dataZ.npy', output)
    return output
...
def from_observation_to_usablestate(states, which_agent, just_one):
    ...
    ######### POINTMASS ###################
    #######################################
    #print("#######################fromobservation#######upup################%$%#%$#$%#$%##########")
    #print(len(states),len(states[0]),len(states[0][0]))
    #print("#######################fromobservation##########downdown#########%$%#%$#$%#$%##########")
